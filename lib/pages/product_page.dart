...
class _ProductPageState extends State<ProductPage> {
  ...
  // [해결] Undefined name 오류 방지를 위한 상태 세트 정의
  ...
  void _syncFiltering(List<ProductModel> rawItems) {
    ...
    List<ProductModel> result = rawItems.where((p) {
      ...
      final isOut = _outboundStatuses.contains(p.status) || _exceptionStatuses.contains(p.status);
      ...
      child: Column(children: [
        Row(children: [
          Expanded(child: Wrap(spacing: 8, runSpacing: 8, children: [
            _buildActionIconButton(Icons.refresh, "새로고침", () { provider.fetchData(); }),
            ...
            _buildActionIconButton(FontAwesomeIcons.fileArrowDown, "엑스포트", () { _exportToExcel(context, filtered); }, color: Colors.green),
            _buildActionIconButton(Icons.settings_outlined, "표시 설정", () { _showColumnSelectionDialog(provider); }),
            _buildActionIconButton(Icons.delete_sweep_outlined, "리셋", () { _showResetDialog(provider); }, color: AppTheme.danger),
          ])),
          _buildActionIconButton(Icons.add_box, "신규 등록", () { _showForm(context, provider, null); }, color: AppTheme.primary, isLarge: true),
        ]),
  ...
  Widget _buildDetailView(ProductProvider provider, String groupName, List<ProductModel> items, {Key? key}) {
    ...
    return Column(key: key, children: [
      Container(padding: const EdgeInsets.all(16), color: Colors.white, child: Row(children: [Container(width: 4, height: 18, color: AppTheme.primary), const SizedBox(width: 8), Text(groupName, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 18))])),
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Row(children: [
                      ...
                      if (!p.isApproved) ...[const SizedBox(width: 8), const Icon(Icons.gpp_maybe, color: AppTheme.danger, size: 18)]
                    ]),
                    const SizedBox(height: 10),
                    Wrap(spacing: 20, runSpacing: 8, children: cols.where((c) => c != '품명').map((c) => SizedBox(width: 140, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(c, style: AppTheme.itemLabelStyle(context)),
                      Text(_getDisplayValue(p, c), style: AppTheme.itemValueStyle(context).copyWith(fontSize: 14))
                    ]))).toList()),
                    const SizedBox(height: 8),
                    Text(p.metadata['last_location_info']?['full_name'] ?? "위치 정보 없음", style: AppTheme.itemLabelStyle(context).copyWith(fontSize: 13, fontWeight: FontWeight.w500))
                  ])),
  ...
  Widget _buildGroupTile(ProductProvider provider, String title, List<ProductModel> items, bool isSelected, bool isMobile) {
    final healthRatio = items.isEmpty ? 0.0 : items.where((i) => !_outboundStatuses.contains(i.status)).length / items.length;
  ...
  // [해결] Async gap 방지를 위한 핸들 캡처 로직 적용
  ...
  Future<void> _processAssetAccess(ProductProvider provider, ProductModel p, String type) async {
    ...
    final success = await provider.handleSave(p: p, data: {
      'status': result['status'], 'location': result['location'], 'is_approved': isApproved,
  ...
  void _confirmGroupDelete(BuildContext ctx, ProductProvider provider, String name, List<ProductModel> items) {
    ...
    showDialog(context: ctx, builder: (c) => AlertDialog(title: AppTheme.dialogTitle("그룹 일괄 삭제", Icons.warning, color: AppTheme.danger), content: Text("[$name] 그룹의 모든 자산(${items.length}개)을 삭제하시겠습니까?"), actions: [AppTheme.actionButton(label: "취소", color: Colors.transparent, textColor: Colors.black54, onPressed: () => navigator.pop()), const SizedBox(width: 8), AppTheme.actionButton(label: "일괄 삭제", color: AppTheme.danger, onPressed: () async { await provider.deleteMultipleProducts(items.map((e) => e.id).toList()); if (mounted) { _syncFiltering(provider.items); navigator.pop(); } })]));
  }

  void _confirmIndividualDelete(BuildContext ctx, ProductProvider provider, ProductModel p) {
    final navigator = Navigator.of(ctx);
    showDialog(context: ctx, builder: (c) => AlertDialog(title: AppTheme.dialogTitle("삭제 확인", Icons.delete), content: Text("[${p.name}] 자산을 삭제하시겠습니까?"), actions: [AppTheme.actionButton(label: "취소", color: Colors.transparent, textColor: Colors.black54, onPressed: () => navigator.pop()), const SizedBox(width: 8), AppTheme.actionButton(label: "삭제", color: AppTheme.danger, onPressed: () async { await provider.deleteMultipleProducts([p.id]); if (mounted) { _syncFiltering(provider.items); navigator.pop(); } })]));
  }
  ...
  Future<void> _showForm(BuildContext context, ProductProvider provider, ProductModel? p, {String? initialTag}) async {
    ...
    final messenger = ScaffoldMessenger.of(context), navigator = Navigator.of(context);
    ...
    await showDialog(context: context, barrierDismissible: false, builder: (ctx) => StatefulBuilder(builder: (dialogCtx, setS) => AlertDialog(
      ...
      content: SizedBox(width: 850, child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
        const SizedBox(height: 12),
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Column(children: [
            ...
            Row(children: [const Text("승인 상태", style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold)), const SizedBox(width: 8), Switch(value: isApproved, activeThumbColor: AppTheme.success, activeTrackColor: AppTheme.success.withValues(alpha: 0.5), onChanged: (v) => setS(() => isApproved = v))])
          ]),
          const SizedBox(width: 24),
          Expanded(child: Column(children: [
            TextField(controller: nameC, style: AppTheme.itemValueStyle(context), decoration: AppTheme.inputDecoration(label: "품명 (필수)", hint: "제품명을 입력하세요", context: context)),
            const SizedBox(height: 16),
            TextField(controller: tagC, style: AppTheme.itemValueStyle(context), decoration: AppTheme.inputDecoration(label: "태그ID (RFID EPC)", context: context)),
          ]))
        ]),
        const SizedBox(height: 24), _buildSectionHeader(Icons.star, "기본 제원 정보", Colors.blue), const SizedBox(height: 20),
        Wrap(spacing: 16, runSpacing: 16, children: [
          SizedBox(width: 370, child: TextField(controller: locC, decoration: AppTheme.inputDecoration(label: "로케이션", context: context))),
          SizedBox(width: 370, child: TextField(controller: catC, decoration: AppTheme.inputDecoration(label: "자산 카테고리", context: context))),
          SizedBox(width: 370, child: TextField(controller: specC, decoration: AppTheme.inputDecoration(label: "모델 규격", context: context))),
          SizedBox(width: 370, child: TextField(controller: safeC, decoration: AppTheme.inputDecoration(label: "안전재고수량", context: context))),
          SizedBox(width: 370, child: TextField(controller: snC, decoration: AppTheme.inputDecoration(label: "S/N", context: context))),
        ...
        if (metaC.isNotEmpty) ...[
          ...
          Wrap(spacing: 16, runSpacing: 16, children: metaC.entries.map((e) {
            return SizedBox(width: 370, child: TextField(controller: e.value, decoration: AppTheme.inputDecoration(label: e.key, context: context)));
          }).toList())
        ],
        if (p == null) ...[
          const SizedBox(height: 32), _buildSectionHeader(Icons.copy_all, "벌크 생성", AppTheme.warning), const SizedBox(height: 20),
          SizedBox(width: 370, child: TextField(controller: qtyC, keyboardType: TextInputType.number, decoration: AppTheme.inputDecoration(label: "등록 수량", hint: "한 번에 생성할 개수", context: context)))
        ]
      ]))),
      actions: [
        ...
        AppTheme.actionButton(label: "통합 저장", onPressed: () async {
          final meta = Map<String, dynamic>.from(p?.metadata ?? {}); metaC.forEach((k, c) => meta[k] = c.text.trim());
          ...
          if (p == null) {
            ...
            for (int i = 0; i < loop; i++) {
              ...
              if (!await provider.handleSave(p: null, data: {...baseData, 'tag_id': tid}, imageXFile: file)) { ok = false; }
            }
          } else { ok = await provider.handleSave(p: p, data: {...baseData, 'tag_id': tagC.text.trim()}, imageXFile: file); }
          if (ok && mounted) { _syncFiltering(provider.items); navigator.pop(); messenger.showSnackBar(const SnackBar(content: Text("성공적으로 저장되었습니다."), elevation: 0)); }
        })
      ],
    )));
  }
  ...
  Widget _buildThumbnail(ProductModel p, {double size = 44}) {
    ...
    return Container(width: size, height: size, decoration: BoxDecoration(color: const Color(0xFFF1F3F5), borderRadius: BorderRadius.circular(10), border: Border.all(color: Colors.black12, width: 1)), clipBehavior: Clip.antiAlias, child: (url.isNotEmpty) ? Image.network(url, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Icon(Icons.broken_image, size: 18)) : const Icon(Icons.inventory_2_outlined, color: Colors.black12, size: 22));
  }

  void _showResetDialog(ProductProvider provider) {
    final navigator = Navigator.of(context);
    showDialog(context: context, builder: (ctx) => AlertDialog(title: AppTheme.dialogTitle("전체 초기화", Icons.delete_forever, color: AppTheme.danger), content: const Text("모든 정보를 삭제하시겠습니까?"), actions: [AppTheme.actionButton(label: "취소", color: Colors.transparent, textColor: Colors.black54, onPressed: () => navigator.pop()), AppTheme.actionButton(label: "삭제", color: AppTheme.danger, onPressed: () async { await provider.resetAllProducts(); if (mounted) { _syncFiltering(provider.items); navigator.pop(); } })]));
  }
  ...
  void _showHistoryDialog(BuildContext context, ProductModel p) {
    showDialog(context: context, builder: (ctx) => AlertDialog(title: AppTheme.dialogTitle('[${p.name}] 상세 이력', Icons.history, color: Colors.blueGrey), content: SizedBox(width: 500, height: 600, child: p.history.isEmpty ? _buildEmptyState("이력이 없습니다.") : ListView.separated(itemCount: p.history.length, separatorBuilder: (_, __) => const Divider(), itemBuilder: (ctx, idx) {
      final log = p.history[idx];
      return ListTile(contentPadding: EdgeInsets.zero, title: Text(log['type'] ?? "-", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)), subtitle: Text("${log['time']} | ${log['location'] ?? '-'} | ${log['handler'] ?? '-'}"), leading: Icon(_statusIcons[log['type']] ?? Icons.history, size: 20, color: AppTheme.primary));
  ...
  void _showColumnSelectionDialog(ProductProvider provider) {
    ...
    showDialog(context: context, builder: (ctx) => StatefulBuilder(builder: (context, setS) => AlertDialog(
      ...
      content: SizedBox(width: 480, child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (metaFields.isEmpty) ...[const Text("추가된 메타데이터가 없습니다.")] else ...[...metaFields.map((k) => _buildSelectionListItem(k, temp, (v) => setS(() => v)))],
      ]))),
  ...
  Widget _buildSelectionListItem(String label, List<String> currentList, Function(void) onChanged) {
    ...
    return Padding(padding: const EdgeInsets.symmetric(vertical: 4), child: InkWell(onTap: () { if (isSelected) { if (currentList.length > 1) { currentList.remove(label); } } else { if (currentList.length < 5) { currentList.add(label); } } onChanged(null); }, borderRadius: BorderRadius.circular(8), child: AnimatedContainer(duration: const Duration(milliseconds: 200), padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14), decoration: BoxDecoration(color: isSelected ? AppTheme.primary.withValues(alpha: 0.05) : Colors.white, borderRadius: BorderRadius.circular(8), border: Border.all(color: isSelected ? AppTheme.primary : Colors.black12, width: isSelected ? 2.5 : 1.0)), child: Row(children: [Icon(isSelected ? Icons.check_circle_rounded : Icons.radio_button_unchecked, size: 20, color: isSelected ? AppTheme.primary : Colors.black26), const SizedBox(width: 16), Expanded(child: Text(label, style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: isSelected ? AppTheme.primary : Colors.black45)))]))));
  }

  Widget _buildColumnGroupHeader(String title) => Row(children: [Container(width: 4, height: 16, decoration: BoxDecoration(color: Colors.blueGrey, borderRadius: BorderRadius.circular(2))), const SizedBox(width: 10), Text(title, style: const TextStyle(fontWeight: FontWeight.w900, color: Colors.blueGrey, fontSize: 14, letterSpacing: -0.5))]);

  Widget _buildCircleAction(IconData icon, Color color, String tip, VoidCallback onTap) => Tooltip(message: tip, child: InkWell(onTap: onTap, borderRadius: BorderRadius.circular(25), child: Container(width: 42, height: 42, decoration: BoxDecoration(color: color.withValues(alpha: 0.08), shape: BoxShape.circle), child: Icon(icon, color: color, size: 20))));
...
class _ManualInoutDialogState extends State<_ManualInoutDialog> {
  late TextEditingController _locC, _reasonC; late String _selS; bool _isApproved = true; String _selectedHandler = "";
  ...
  @override Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      content: SizedBox(width: 450, child: Column(mainAxisSize: MainAxisSize.min, children: [
        const SizedBox(height: 20),
        DropdownButtonFormField<String>(initialValue: _selS, decoration: AppTheme.inputDecoration(label: "작업 선택", context: context), items: (widget.type == '수기입고' ? ['보유중', '수동입고', '회수/반납'] : ['수동출고', '판매/배송출고', '폐기']).map((v) => DropdownMenuItem(value: v, child: Text(v))).toList(), onChanged: (v) => setState(() => _selS = v!)),
        const SizedBox(height: 16),
        TextField(controller: _locC, decoration: AppTheme.inputDecoration(label: "위치", context: context)),
        const SizedBox(height: 16),
        Autocomplete<String>(optionsBuilder: (val) => workerList.where((o) => o.contains(val.text)), onSelected: (s) => _selectedHandler = s, fieldViewBuilder: (ctx, ctrl, focus, __) => TextField(controller: ctrl, focusNode: focus, decoration: AppTheme.inputDecoration(label: "담당 작업자", context: context))),
        const SizedBox(height: 20),
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(_isApproved ? "승인됨" : "미승인", style: TextStyle(color: _isApproved ? AppTheme.success : AppTheme.danger, fontWeight: FontWeight.bold)), Switch(value: _isApproved, activeThumbColor: AppTheme.success, activeTrackColor: AppTheme.success.withValues(alpha: 0.5), onChanged: (v) => setState(() => _isApproved = v))]),
      ])),
