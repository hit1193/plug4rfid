...
class _PersonPageState extends State<PersonPage> {
  ...
  Map<String, dynamic> _calculateMetrics(List<Person> list) {
    ...
    int todayIn = 0, todayOut = 0, currentRemained = 0;
  ...
  Future<void> _processAccessWithLocation(PersonProvider provider, Person p, String type) async {
    ...
    // 3. 작업 완료 후 다시 mounted 체크 및 캡처된 messenger 사용
    if (success && mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    final filteredList = provider.list.where((p) {
      ...
      if (_activeMetricFilter == "당일 입장") return (p.metadata['last_access_type'] == '입장' && lastTime.startsWith(todayStr));
      if (_activeMetricFilter == "당일 퇴장") return (p.metadata['last_access_type'] == '퇴장' && lastTime.startsWith(todayStr));
      if (_activeMetricFilter == "현재 잔류") return p.metadata['last_access_type'] == '입장';
      return true;
    }).toList();
  ...
  Widget _buildListView(List<Person> list, PersonProvider provider, List<String> columns, ThemeData theme) {
    if (list.isEmpty) return _buildEmptyState("데이터가 없습니다.");
          ...
          child: Container(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  // [중앙 테마 연동] 굵고 진한 다크그레이 데이터 폰트
                  Text(item.name, style: AppTheme.itemValueStyle(context).copyWith(fontSize: 18)),
                  const SizedBox(width: 12),
                  _buildStatusBadge(status),
                  if (!item.isApproved) ...[const SizedBox(width: 8), const Icon(Icons.gpp_maybe, color: AppTheme.danger, size: 18)]
                ]),
                const SizedBox(height: 10),
                Wrap(spacing: 20, runSpacing: 8, children: columns.map((col) => SizedBox(width: 140, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  // [중앙 테마 연동] Silver 색상의 레이블 폰트
  ...
  String _getMetaValue(Person item, String key) {
    ...
    if (baseFields.containsKey(key)) return baseFields[key]!;
    return item.metadata[key]?.toString() ?? "-";
  }

  // --- 다이얼로그 호출 로직 (생략 없이 완결) ---
  ...
  void _showHistoryDialog(BuildContext context, Person p, ThemeData theme) {
    ...
    showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      content: SizedBox(width: 550, height: 600, child: history.isEmpty ? _buildEmptyState("기록 없음") : ListView.separated(padding: const EdgeInsets.symmetric(vertical: 10), itemCount: history.length, separatorBuilder: (_, __) => const SizedBox(height: 10), itemBuilder: (c, i) {
        final log = history[i]; final String type = log['type'] ?? "-"; final bool approved = log['is_approved'] ?? true;
        Color col = approved ? (type == '입장' ? AppTheme.success : AppTheme.warning) : AppTheme.danger;
        return Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: theme.cardTheme.color, borderRadius: BorderRadius.circular(10), border: Border.all(color: col.withValues(alpha: 0.15))), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  void _showColumnSelectionDialog(PersonProvider provider, ThemeData theme) {
    ...
    for (final p in provider.list) {
      for (final k in p.metadata.keys) {
        if (!_excludedSystemKeys.contains(k) && !k.endsWith('_internal')) keySet.add(k);
      }
    }
    ...
    showDialog(context: context, builder: (ctx) => StatefulBuilder(builder: (context, setS) => AlertDialog(
      ...
      content: SizedBox(width: 480, child: available.isEmpty ? const Text("추가 필드 없음") : SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: available.map((key) {
        final bool sel = temp.contains(key);
        return Padding(padding: const EdgeInsets.symmetric(vertical: 4), child: InkWell(onTap: () => setS(() => sel ? (temp.length > 1 ? temp.remove(key) : null) : (temp.length < 5 ? temp.add(key) : null)), borderRadius: BorderRadius.circular(8), child: AnimatedContainer(duration: const Duration(milliseconds: 200), padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14), decoration: BoxDecoration(color: sel ? theme.colorScheme.primary.withValues(alpha: 0.05) : Colors.transparent, borderRadius: BorderRadius.circular(8), border: Border.all(color: sel ? theme.colorScheme.primary : Colors.black.withValues(alpha: 0.15), width: sel ? 2.5 : 1.0)), child: Row(children: [Icon(sel ? Icons.check_circle_rounded : Icons.radio_button_unchecked, size: 20, color: sel ? theme.colorScheme.primary : Colors.black26), const SizedBox(width: 16), Expanded(child: Text(key, style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: sel ? theme.colorScheme.primary : Colors.black45)))]))));
      }).toList()))),
      actions: [AppTheme.actionButton(label: "취소", color: Colors.transparent, textColor: theme.colorScheme.onSurface.withValues(alpha: 0.5), onPressed: () => Navigator.pop(ctx)), AppTheme.actionButton(label: "설정 적용", onPressed: () async { await provider.saveRemoteSettings(temp); if (mounted) Navigator.pop(ctx); })],
    )));
  }

  Future<void> _showResetConfirmationDialog(PersonProvider provider, ThemeData theme) async {
    final messenger = ScaffoldMessenger.of(context);
    final confirm = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(title: AppTheme.dialogTitle("삭제 확인", Icons.warning, color: AppTheme.danger), content: const Text("서버의 모든 정보를 영구 삭제하시겠습니까?"), actions: [AppTheme.actionButton(label: "취소", color: Colors.transparent, textColor: theme.colorScheme.onSurface.withValues(alpha: 0.5), onPressed: () => Navigator.pop(ctx, false)), AppTheme.actionButton(label: "삭제 실행", color: AppTheme.danger, onPressed: () => Navigator.pop(ctx, true))]));
    if (confirm == true && mounted) { await provider.resetAllPersons(); messenger.showSnackBar(const SnackBar(content: Text('초기화 완료'))); }
  }

  Future<void> _handleBatchImport(PersonProvider provider) async {
    final messenger = ScaffoldMessenger.of(context);
    final count = await provider.batchImportFromExcel();
    if (count > 0 && mounted) messenger.showSnackBar(SnackBar(content: Text('$count명 등록 완료')));
  }
  ...
  Future<void> _exportToExcel(List<Person> dataList) async {
    ...
    final messenger = ScaffoldMessenger.of(context);
    try {
      final excel = excel_pkg.Excel.createExcel(); final sheet = excel['인원리스트']; excel.rename('Sheet1', '인원리스트');
      final headers = ['성명', '사번', '부서', '태그ID'];
      for (int i = 0; i < headers.length; i++) sheet.cell(excel_pkg.CellIndex.indexByColumnRow(columnIndex: i, rowIndex: 0)).value = excel_pkg.TextCellValue(headers[i]);
      for (int r = 0; r < dataList.length; r++) {
        final p = dataList[r]; final row = [p.name, p.code, p.department, p.tagId];
        for (int c = 0; c < row.length; c++) sheet.cell(excel_pkg.CellIndex.indexByColumnRow(columnIndex: c, rowIndex: r + 1)).value = excel_pkg.TextCellValue(row[c]);
      }
      ...
      if (path != null) { await File(path).writeAsBytes(excel.encode()!); if (mounted) messenger.showSnackBar(const SnackBar(content: Text('✅ 엑셀 저장 완료'))); }
    } catch (e) { if (mounted) messenger.showSnackBar(SnackBar(content: Text('오류: $e'))); }
  }
  ...
  Future<void> _showForm(PersonProvider provider, Person? p, ThemeData theme) async {
    ...
    bool approved = p?.isApproved ?? true; XFile? file; Uint8List? preview; final Map<String, TextEditingController> metaC = {};
    if (p != null) p.metadata.forEach((k, v) { if (!_excludedSystemKeys.contains(k) && !k.endsWith('_internal') && v is! Map && v is! List) metaC[k] = TextEditingController(text: v?.toString() ?? ""); });
    ...
    showDialog(context: context, barrierDismissible: false, builder: (ctx) => StatefulBuilder(builder: (dialogCtx, setS) => AlertDialog(
        ...
        content: SizedBox(width: 900, child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Column(children: [
              GestureDetector(onTap: () async { final img = await ImagePicker().pickImage(source: ImageSource.gallery); if (img != null) { final b = await img.readAsBytes(); setS(() { file = img; preview = b; }); } }, child: Container(width: 180, height: 210, decoration: BoxDecoration(color: theme.cardTheme.color, borderRadius: BorderRadius.circular(15), border: Border.all(color: theme.dividerTheme.color ?? Colors.grey, width: 2)), child: Center(child: preview != null ? Image.memory(preview!, fit: BoxFit.cover) : (p?.getImageUrl(widget.baseUrl) != null ? Image.network(p!.getImageUrl(widget.baseUrl)!, fit: BoxFit.cover) : const Icon(Icons.camera_alt, size: 40, color: Colors.grey))))),
              const SizedBox(height: 16),
              Row(children: [const Text("출입 승인", style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold)), const SizedBox(width: 8), Switch(value: approved, activeThumbColor: AppTheme.success, activeTrackColor: AppTheme.success.withValues(alpha: 0.5), onChanged: (v) => setS(() => approved = v))])
            ]),
          ...
          if (metaC.isNotEmpty) ...[const SizedBox(height: 32), const Divider(), const SizedBox(height: 16), Wrap(spacing: 16, runSpacing: 16, children: metaC.entries.map((e) => SizedBox(width: 360, child: _buildTextField(e.value, e.key, theme))).toList())]
        ]))),
        actions: [AppTheme.actionButton(label: "취소", color: Colors.transparent, textColor: theme.colorScheme.onSurface.withValues(alpha: 0.5), onPressed: () => Navigator.pop(dialogCtx)), AppTheme.actionButton(label: "통합 저장", onPressed: () async {
          ...
          if (await provider.handleSave(p: p, data: data, imageXFile: file) && mounted) navigator.pop();
        })]
    )));
  }
  ...
  void _confirmDelete(PersonProvider provider, Person p, ThemeData theme) {
    ...
    showDialog(context: context, builder: (c) => AlertDialog(title: AppTheme.dialogTitle("삭제 확인", Icons.delete), content: Text("[${p.name}] 정보를 삭제하시겠습니까?"), actions: [AppTheme.actionButton(label: "취소", color: Colors.transparent, textColor: theme.colorScheme.onSurface.withValues(alpha: 0.5), onPressed: () => Navigator.pop(c)), AppTheme.actionButton(label: "삭제 실행", color: AppTheme.danger, onPressed: () async { if (await provider.deletePerson(p.id) && mounted) nav.pop(); })]));
  }
}
...
class _LocationSelectionDialog extends StatefulWidget {
  final String type; final List<Person> existingPersons;
...
class _LocationSelectionDialogState extends State<_LocationSelectionDialog> {
  late List<String> _buildings, _gates; final _bC = TextEditingController(), _gC = TextEditingController(); bool _ok = true;
  @override void initState() {
    super.initState();
    final Set<String> b = {'본관A', '공장B', '물류창고C', '연구소D'}, g = {'정문G1', '후문G2', '하차장G3', '비상구G4'};
    for (var p in widget.existingPersons) {
      final loc = p.metadata['last_location_info'];
      if (loc is Map) { if (loc['building'] != null) b.add(loc['building']); if (loc['gate'] != null) g.add(loc['gate']); }
    }
    _buildings = b.toList()..sort(); _gates = g.toList()..sort(); _bC.text = _buildings.first; _gC.text = _gates.first;
  }
  @override Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      content: SizedBox(width: 420, child: Column(mainAxisSize: MainAxisSize.min, children: [
        const SizedBox(height: 20), // 타이틀과의 여백 확보
        ...
        Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), decoration: BoxDecoration(color: _ok ? AppTheme.success.withValues(alpha: 0.05) : AppTheme.danger.withValues(alpha: 0.05), borderRadius: BorderRadius.circular(10), border: Border.all(color: _ok ? AppTheme.success.withValues(alpha: 0.2) : AppTheme.danger.withValues(alpha: 0.2))), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(_ok ? "승인됨" : "미승인", style: TextStyle(fontWeight: FontWeight.bold, color: _ok ? AppTheme.success : AppTheme.danger)), Switch(value: _ok, activeThumbColor: AppTheme.success, activeTrackColor: AppTheme.success.withValues(alpha: 0.5), onChanged: (v) => setState(() => _ok = v))]))
      ])),
      actions: [AppTheme.actionButton(label: "취소", color: Colors.transparent, textColor: theme.colorScheme.onSurface.withValues(alpha: 0.5), onPressed: () => Navigator.pop(context)), AppTheme.actionButton(label: "위치 확정", onPressed: () => Navigator.pop(context, {'building': _bC.text, 'gate': _gC.text, 'is_approved': _ok}))],
    );
  }
  Widget _buildCombo(String label, TextEditingController ctrl, List<String> opts, ThemeData theme) {
    return Autocomplete<String>(optionsBuilder: (v) => v.text == '' ? opts : opts.where((o) => o.contains(v.text)), onSelected: (s) => ctrl.text = s, fieldViewBuilder: (ctx, tC, fN, __) {
